...
If you would like to use the USC-CMU, CMU250, or any other keyword lists located in the Data folder in this repository, then no extra work is required!
...
With this code, we added words from the PWG list (that did not have weights assigned) that were included in the CMU1000 list using left-joins from the `dplyr` library.

In addition, this R file has code to add a column to your initial un-weighted keyword list which records the dataset each word was mapped to.
...
At USC, we received a file called `USC_FULL_COURSE_LIST_20182_20183_20191` which can be found in the Data folder. This csv file contains data for every single course offered for 3 different semesters at USC. The first thing we did was clean up the data, get rid of repeats (and add a separate columns to keep track of how many sections were offered), and rename the columns.

The relevant column names in this file were 'Rcl Course' which was renamed 'course_title', 'Rcl Term' which was renamed to 'semester' using a function that gets the semester based on the term (format is YYYYS), 'Rcl Class' which we renamed to 'course_num', 'Rcl Title' which we renamed to 'course', 'Rca Desc' which was renamed to 'course_desc', and 'RCL_DEPT' which was renamed to 'department'. One column called 'Exclude Flag' has 0's and 1's to indicate whether that course should be included in the data analysis. Excluded courses (with a 1 in that column) were mostly research courses that do not count toward AC-1 credit. This file contains other data such as the course level and the number of students, but these were not relevant to our mapping.

It can be expected that others using this package will have different formats for their course data, so just follow the R file `cleaning_course_data.R` and adjust column specifications to fit your needs. The problem I expect people to encounter most frequently is the semester/term formatting. The function 'get_semesters' takes a specific format to return the semester to which that term refers, which will most likely be different for each school's data. If your data already has a semester column you are satisfied with, then ignore that function.

In the R file named `cleaning_course_data.R`, you can see code for how to clean this course data. The three functions to do these are `clean_data`, `get_semesters`, and `transform_data` which gets the course data into a nice format. The produced course data can be found in the Data folder with the name `usc_courses`.


## Mapping course descriptions to 17 SDGs

In the R file named `mapping_course_descriptions.R`, you can find code for mapping the clean course data to the 17 SDG's using loops and a function called `tabulate_sdg_keywords` (which can be found in the file named after that function).

Before running the mapping code, be sure to run `tabulate_sdg_keywords.R`, and don't forget to read in your keyword list before running that function If you created your own keyword list, you need to read it in and update the body this function to account for the name of yours since you cannot pass a csv file directly into the function parameter. Look at the comments in the file and follow the example of how I added the parameter "cmu_usc" to the function. 

When you run the `mapping_course_descriptions.R` (this may take awhile), ensure that the `keywords` parameter is set to the keyword list of your choosing (by the name you created in the body of the function from above). By default, it is set to the Elsevier top 100 keywords list. Since this function takes so long to run, it might be a good idea to save this dataframe as an R object you can load in later without rerunning the file. To do this, 
<!-- chang the default to be ours? -->
